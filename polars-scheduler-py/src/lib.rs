...
use pyo3_polars::{
    ...
    export::polars_core::{
        prelude::*,
        chunked_array::builder::ListBuilder,
    },
};

use scheduler_core::{
    Entity, WindowSpec, ConstraintExpr, ConstraintType, ConstraintRef, Frequency,
    SchedulerConfig, ScheduleStrategy, ScheduleResult, ScheduledEvent,
    parse_from_table, parse_one_constraint, parse_one_window, solve_schedule,
...
fn schedule_dataframe(
    ...
    Python::with_gil(|py| {
        ...
        let df = if pyo3_polars::is_polars_dataframe(df) {
            ...
            let to_polars = PyModule::import(py, "polars")?
                .getattr("from_pandas")?;
            to_polars.call1((df,))?
        };
        ...
        let entities = df_to_entities(&pldf).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Error converting DataFrame to entities: {}", e))
        })?;
        ...
        let strategy = match strategy.to_lowercase().as_str() {
            ...
            _ => {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                    format!("Invalid strategy: {}. Must be 'earliest' or 'latest'", strategy)
                ));
            }
        };
            ...
            window_specs.iter()
                ...
                .map_err(|e| {
                    PyErr::new::<pyo3::exceptions::PyValueError, _>(
                        format!("Error parsing windows: {}", e)
                    )
                })?
        } else {
            Vec::new()
        ...
        let constraints = if df.schema().contains("Constraints") {
            match df.column("Constraints") {
                Ok(c) => {
                    match c.dtype() {
                        DataType::List(_) => {
                            let list_arr = c.list().map_err(|_| "Failed to convert Constraints to list".to_string())?;
                                    ...
                                    for i in 0..constr_arr.len() {
                                        if let Some(constr_str) = constr_arr.get_str(i) {
                                            match parse_one_constraint(constr_str) {
                                                ...
                                                Err(e) => return Err(format!("Error parsing constraint '{}': {}", constr_str, e)),
                                            }
                                        }
                                    }
                                    constraints
        ...
        let windows = if df.schema().contains("Windows") {
            match df.column("Windows") {
                Ok(w) => {
                    match w.dtype() {
                        DataType::List(_) => {
                            let list_arr = w.list().map_err(|_| "Failed to convert Windows to list".to_string())?;
                                    ...
                                    for i in 0..win_arr.len() {
                                        if let Some(win_str) = win_arr.get_str(i) {
                                            match parse_one_window(win_str) {
                                                ...
                                                Err(e) => return Err(format!("Error parsing window '{}': {}", win_str, e)),
                                            }
                                        }
                                    }
                                    windows
