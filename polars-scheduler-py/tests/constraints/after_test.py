...
def test_after_constraint():
    ...
    print(f"Porridge time: {porridge_hour}h, Creatine time: {creatine_hour}h")
...
def test_after_constraint_with_category():
    ...
    assert is_after_breakfast or is_after_lunch, (
        "Vitamin should be at least 1h after one food item"
    )
...
def test_after_constraint_multiple_daily():
    ...
    assert is_after_any_meal, (
        "Vitamin should be at least 1h after at least one meal instance"
    )
