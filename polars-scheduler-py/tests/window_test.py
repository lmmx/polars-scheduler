...
def test_range_time_window():
    ...
    lunch_time = earliest.filter(pl.col("entity_name") == "lunch").select("time_minutes").item()
    assert lunch_time == 720  # 12:00
    
    # With latest strategy
    latest = scheduler.schedule(strategy="latest")
    lunch_time = latest.filter(pl.col("entity_name") == "lunch").select("time_minutes").item()
...
def test_multiple_windows():
    ...
    earliest = scheduler.schedule(strategy="earliest")
    shake_time = earliest.filter(pl.col("entity_name") == "shake").select("time_minutes").item()
    assert shake_time == 480  # 08:00
    
    # With latest strategy, should pick last window
    latest = scheduler.schedule(strategy="latest")
    shake_time = latest.filter(pl.col("entity_name") == "shake").select("time_minutes").item()
